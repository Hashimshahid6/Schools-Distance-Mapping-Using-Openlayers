fetch('https://www.esef.gkp.pk/schools_map_api/api.php')
  ...
  .then(data => {
    if (data != null) {
      ...
      const schools = data.map(school => ({
        ...
        tehsil: school.tehsil_name,
        uc: school.uc_name,
        vc: school.vc_name,
        name: school.sch_name,
        type: school.sch_type,
        coordinates: [parseFloat(school.y_cord), parseFloat(school.x_cord)]
      }))
      console.log(schools);
      ...
      // For each school, create a new feature and add it to the vector source
      schools.forEach(function (school) {
        if (school.coordinates[0] != 0 && school.coordinates[0] != null && !isNaN(school.coordinates[0])) {
          // console.log(school.coordinates);
          var feature = new ol.Feature({
            geometry: new ol.geom.Point(ol.proj.fromLonLat(school.coordinates)),
            name: school.name,
      ...
      // Create a vector layer for the schools
    ...
    //     }
    //   });
      var GCS = new ol.layer.Vector({
        title: 'GCS',
        source: vectorSource,
        style: function (feature) {
          var type = feature.get('type');
          var iconSrc = type === 'GCS' ? 'GCS.png' : 'icon.png';
          return new ol.style.Style({
            image: new ol.style.Icon({
              anchor: [0.5, 46],
              anchorXUnits: 'fraction',
      ...
      var BECS = new ol.layer.Vector({
        title: 'BECS',
        source: vectorSource,
        style: function (feature) {
          var type = feature.get('type');
          var iconSrc = type === 'BECS' ? 'BECS.png' : 'icon.png';
          return new ol.style.Style({
            image: new ol.style.Icon({
              anchor: [0.5, 46],
              anchorXUnits: 'fraction',
      ...
      var NCHD = new ol.layer.Vector({
        title: 'NCHD',
        source: vectorSource,
        style: function (feature) {
          var type = feature.get('type');
          var iconSrc = type === 'NCHD' ? 'NCHD.png' : 'icon.png';
          return new ol.style.Style({
            image: new ol.style.Icon({
              anchor: [0.5, 46],
              anchorXUnits: 'fraction',
      ...
      var HCIP = new ol.layer.Vector({
        title: 'HCIP',
        source: vectorSource,
        style: function (feature) {
          var type = feature.get('type');
          var iconSrc = type === 'HCIP' ? 'HCIP.png' : 'icon.png';
          return new ol.style.Style({
            image: new ol.style.Icon({
              anchor: [0.5, 46],
              anchorXUnits: 'fraction',
      ...
      var ESS = new ol.layer.Vector({
        title: 'ESS',
        source: vectorSource,
        style: function (feature) {
          var type = feature.get('type');
          var iconSrc = type === 'ESS' ? 'ESS.png' : 'icon.png';
          return new ol.style.Style({
            image: new ol.style.Icon({
              anchor: [0.5, 46],
              anchorXUnits: 'fraction',
      ...
      var NSI = new ol.layer.Vector({
        title: 'NSI',
        source: vectorSource,
        style: function (feature) {
          var type = feature.get('type');
          var iconSrc = type === 'NSI' ? 'NSI.png' : 'icon.png';
          return new ol.style.Style({
            image: new ol.style.Icon({
              anchor: [0.5, 46],
              anchorXUnits: 'fraction',
...
var map = new ol.Map({
    ...
    view: new ol.View({
      center: ol.proj.fromLonLat([71.4200932, 33.9688983]),
      zoom: 8
    }),
...
// function for loading query panel
function query() {
    $('#table').empty();
    if (geojson) {
    ...
    //map.getView().fit(geojson.getSource().getExtent(), map.getSize());
...
measuretype.onchange = function() {
    ...
    map.removeInteraction(draw);
    addInteraction();
    if (measuretype.value == 'area' || measuretype.value == 'length' || measuretype.value == 'clear') {
        popupElement.style.display = 'none'; // Hide popup
    } else {
        popupElement.style.display = 'block'; // Show popup
    }
};
...
// document.addEventListener('DOMContentLoaded', function () {
    moveToCoordinates = function () {
        ...
        if (!isNaN(x) && !isNaN(y)) {
            const coordinates = ol.proj.fromLonLat([y, x]);
            ...
            const markerStyle = new ol.style.Style({
                image: new ol.style.Icon({
                    src: 'pin.png', // Path to your pin image
                    anchor: [0.5, 1],
                    scale: 0.5,
                }),
            });
...
// Create a popup overlay and add it to the map
var popup = new ol.Overlay({
  element: popupElement,
  positioning: 'bottom-center',
  stopEvent: false,
  offset: [0, -50] // Offset to position the popup below the marker
});
map.addOverlay(popup);
// Display the popup when the user clicks a feature
map.on('click', function (event) {
    if(popupElement){
    // Hide existing popup and reset it's position
    popup.setPosition(undefined);
    // Get the feature at the clicked position (if any
    }
    ...
    if (feature) {
      ...
      popupElement.innerHTML = '<p>Name: ' + feature.get('name') + '</p>'
      popupElement.innerHTML += '<p>District: ' + feature.get('district') + '</p>'
      popupElement.innerHTML += '<p>Tehsil: ' + feature.get('tehsil') + '</p>'
      popupElement.innerHTML += '<p>UC: ' + feature.get('uc') + '</p>'
      popupElement.innerHTML += '<p>VC: ' + feature.get('vc') + '</p>'
      popupElement.innerHTML += '<p>Type: ' + feature.get('type') + '</p>'
      popupElement.innerHTML += '<p>Coordinates: ' + feature.get('coordinates')[1] + ', ' + feature.get('coordinates')[0] + '</p>';
      popupElement.innerHTML += '<p><a href="https://www.google.com/maps/search/?api=1&query=' + feature.get('coordinates')[1] + ',' + feature.get('coordinates')[0] + '" target="_blank">Open in Google Maps</a></p>';
    } else {
      popup.setPosition(undefined);
    }
  });
